...
PLACE_ORDER = True
while PLACE_ORDER:
    # Get the menu category from the customer
    ...
    for key in menu:
        print(f"{i}: {key}")
        menu_items[i] = key  # Store category for later retrieval
        i += 1
    ...
    while True:
        ...
        keep_ordering = keep_ordering.upper()  # Convert the input to uppercase for consistency

        if keep_ordering == "N":
            # Exit the loop to stop taking orders
            PLACE_ORDER = False
            break
